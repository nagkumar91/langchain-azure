...
def build_analyst_graph(eval_optimize_graph: Any) -> Any:
    ...
    def call_eval_optimize(state: AnalystState) -> dict[str, Any]:
        ...
        eval_output = eval_optimize_graph.invoke(
            eval_input,
            config={
                "metadata": {
                    "agent_name": f"EvalOptimizer:{section.area}",
...
def build_orchestrator_graph(*, parallel: bool = True) -> Any:
    ...
    def analyst_worker(state: WorkerInput) -> dict[str, Any]:
        ...
        analyst_output = analyst_graph.invoke(
            analyst_input,
            config={
                "metadata": {
                    "agent_name": f"Analyst:{section.analyst_type}",
                    "agent_type": section.analyst_type,
                },
            },
        )
